...
def get_all_transaction_records():
    transaction_records = TransactionRecord.query.filter((TransactionRecord.payer_id == current_user.get_id()) | (
...
def create_transaction():
    ...
    if form.validate_on_submit():
        ...
        friend1 = Friend.query.filter(Friend.user_id == transaction_record.recipient_id,
                                      Friend.friend_id == transaction_record.payer_id).first()
        friend2 = Friend.query.filter(Friend.user_id == transaction_record.payer_id,
                                      Friend.friend_id == transaction_record.recipient_id).first()
