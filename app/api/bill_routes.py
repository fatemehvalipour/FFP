...
def add_bill():
    ...
    if form.validate_on_submit():
        ...
        for friend_id in all_friend_ids:
            ...
            friend1 = Friend.query.filter(
                Friend.user_id == bill.owner_id, Friend.friend_id == new_expense.payer_id).first()
            friend2 = Friend.query.filter(
                Friend.user_id == new_expense.payer_id, Friend.friend_id == bill.owner_id).first()
...
def deleteBill(billId):
    ...
    for expense in expenses_to_update_balances:
        if bill.owner_id == expense["payer_id"]:
            continue
        friend1 = Friend.query.filter(
            Friend.user_id == bill.owner_id, Friend.friend_id == expense["payer_id"]).first()
        friend2 = Friend.query.filter(
            Friend.user_id == expense["payer_id"], Friend.friend_id == bill.owner_id).first()
...
def editBill(billId):
    ...
    if form.validate_on_submit():
        ...
        user_expense = Expense.query.filter(
            Expense.bill_id == bill.id, Expense.payer_id == current_user.get_id()).first()
        user_expense.initial_charge = divvyed_expense
        db.session.commit()

        for friend_id in all_friend_ids:
            friend_expense = Expense.query.filter(
                Expense.payer_id == friend_id, Expense.bill_id == bill.id).first()
            if friend_expense:
                ...
                friend1 = Friend.query.filter(
                    Friend.user_id == bill.owner_id, Friend.friend_id == friend_expense.payer_id).first()
                friend2 = Friend.query.filter(
                    Friend.user_id == friend_expense.payer_id, Friend.friend_id == bill.owner_id).first()
        ...
        if len(all_friend_ids) > 0:
            expense_to_remove = Expense.query.filter(
                Expense.bill_id == bill.id, ~Expense.payer_id.in_(all_friend_ids)).all()
